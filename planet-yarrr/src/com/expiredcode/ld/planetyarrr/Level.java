...
public class Level extends GameScreen
{
	...
	{	Gdx.app.log("a", "1");
		...
		pship = new PlayerShip(a,angolo,speed,190);					//Gdx.app.log("a", "2");
		...
		p.setOrigin(1/2, 7/2);					//Gdx.app.log("a", ""+ii++);
		
		sfondo = new Texture(Gdx.files.internal("data/sfondo.png"));		
		
		loot = new Texture(Gdx.files.internal("data/loot.png"));
		
		boom = new Texture(Gdx.files.internal("data/boom.png"));					//Gdx.app.log("a", "3");//Gdx.app.log("a", ""+ii++);
		...
		pyarrr = new Texture(Gdx.files.internal("data/pyarrr.png"));					//Gdx.app.log("a", "4"+ii++);
		...
		py = new Collider("pyarr",1352,1700,289,289);					//Gdx.app.log("a", "5"+ii++);
	...
	public void render(float deltaTime) 
	{
		...
		batch.draw(sfondo, -3500, -3500,15000,7000);
	...
	void updateScene(float deltaTime)
	{
		...
		{	
			//ii = 480;
			touchpos.x = Gdx.input.getX(j);	touchpos.y = WORLD_HEIGHT-Gdx.input.getY(j);//-243;
			//angolo = touchpos.sub(a.getX(), a.getY()).angle();			
			touch = new Collider("touch",touchpos.x,touchpos.y,70,70);
			
			Gdx.app.log("a", "x"+touchpos.x+" y"+touchpos.y);//	Gdx.app.log("a",Gdx.app.getGraphics().getDisplayModes().toString());
			
			if(touchpos.x>84&&touchpos.x<160&&touchpos.y>26&&touchpos.y<88)//Box2BoxCollision.B2BCollision(touch,mLeft))
			{
				pship.rotateLeft(deltaTime);
			}
			if(touchpos.x>165&&touchpos.x<250&&touchpos.y>26&&touchpos.y<88)//Box2BoxCollision.B2BCollision(touch,mRight))
			{
				pship.rotateRight(deltaTime);
			}
			if(touchpos.x>1100&&touchpos.x<1210+ii&&touchpos.y>26&&touchpos.y<88)//Box2BoxCollision.B2BCollision(touch,mForw))
			{
				pship.moveForward();
			}
			if(touchpos.x>990+ii&&touchpos.x<1095+ii&&touchpos.y>26&&touchpos.y<88)//Box2BoxCollision.B2BCollision(touch,fireUp))
			{
